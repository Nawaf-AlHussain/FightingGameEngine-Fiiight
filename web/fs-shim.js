        ...
        const fsShim = {
                ...
                open(path, flags, mode, callback) {
                        ...
                        try {
                                ...
                                // Assets CDN fallback: if the file is under chars/ or stages/
                                // and doesn't exist in the VFS, try fetching it from GitHub.
                                if (window.__assetsCDN && window.__assetsCDN.isAssetPath(path)) {
                                        const cleanPath = path.replace(/^\/ikemen\//, "").replace(/^\//, "");
                                        window.__assetsCDN.fetchAndInject(cleanPath).then((data) => {
                                                if (data) {
                                                        // Inject the fetched data into the VFS
                                                        const parts = cleanPath.split("/");
                                                        let dir = root;
                                                        const cwdPath = cwd.replace(/^\//, "").split("/").filter(Boolean);
                                                        for (const p of cwdPath) {
                                                                if (!dir.children.has(p)) dir.children.set(p, makeDir());
                                                                dir = dir.children.get(p);
                                                        }
                                                        for (let i = 0; i < parts.length - 1; i++) {
                                                                if (!dir.children.has(parts[i])) dir.children.set(parts[i], makeDir());
                                                                dir = dir.children.get(parts[i]);
                                                        }
                                                        const fileName = parts[parts.length - 1];
                                                        dir.children.set(fileName, makeFile(data));
                                                        // Retry the open now that the file exists
                                                        try {
                ...
                stat: cbWrap((path) => {
                        try {
                                return statFor(getNode(path).node);
                        } catch (err) {
                                // Assets CDN: if stat fails for a chars/ or stages/ path,
                                // check the manifest and return a fake stat if it exists.
                                if (window.__assetsCDN && window.__assetsCDN.isAssetPath(path)) {
                                        const cleanPath = path.replace(/^\/ikemen\//, "").replace(/^\//, "");
                                        const parts = cleanPath.split("/");
                ...
                readdir: cbWrap((path) => {
                        try {
                                ...
                                // Assets CDN: if readdir fails for chars/, return all character IDs
                                if (window.__assetsCDN && window.__assetsCDN.isAssetPath(path)) {
                                        const cleanPath = path.replace(/^\/ikemen\//, "").replace(/^\//, "");
                                        if (cleanPath === "chars" || cleanPath === "chars/") {
                                                return window.__assetsCDN.getCharacters().map(c => c.id);
                                        }
                                        if (cleanPath === "stages" || cleanPath === "stages/") {
                                                const stages = window.__assetsCDN.getStages();
                                                const files = [];
                                                for (const s of stages) {
                                                        for (const f of (s.files || [s.id])) {
                                                                if (f.endsWith(".def")) files.push(f);
                                                        }
                                                }
                                                return files;
                                        }
