...
(() => {
        ...
        async function boot() {
                ...
                // Assets CDN: fetch manifest and generate select.def with all
                // characters/stages from the Assets repo.
                if (window.__assetsCDN) {
                        setStatus("Fetching character roster…");
                        await window.__assetsCDN.fetchManifest();
                        const selectDef = window.__assetsCDN.generateSelectDef();
                        // Write the generated select.def into the VFS, overlaying the bundled one
                        const enc = new TextEncoder();
                        globalThis.fs.writeFileSync("/ikemen/data/select.def", enc.encode(selectDef));
                        console.log("[assets-cdn] select.def generated with",
                                window.__assetsCDN.getCharacters().length, "characters,",
                                window.__assetsCDN.getStages().length, "stages");
                }
